...
const getBodyBGColor = async (pg: Page) => {
...
const toggleThumbs = async (page: Page, checkboxSelector: string) => {
  const THUMB_ITEMS = "[data-testid=home-page] > div > div";
  const MAKE_SELECTION = "home-nothumbs";
  const ms = page.getByTestId(MAKE_SELECTION);
  ...
  const ms2 = page.getByTestId(MAKE_SELECTION);
...
test("mobile: open nav, toggle thumbs, close nav", async ({ page }) => {
  ...
  const accordionDetail = page.locator(ACCORDIAN_DETAIL);
  await expect(accordionDetail).toHaveCount(1);
